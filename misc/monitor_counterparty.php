...
<?php
/*********************************************************************
 * monitor-unoparty.php 
 * 
 * Script to handle monitoring a Unoparty API endpoint for the 
 * 'Unoparty database is behind backend.' error message and then
...
$server  = ($testnet) ? 'unoparty-testnet' : 'unoparty';
...
// Try to connect to the Unoparty API and run get_running_info
// Status : 0=Down, 1=UP, 2=Lagging
function check_unoparty_status(){
    $status = 0;
    $ch   = curl_init();
    $data = array(
        'id'      => mt_rand(),
    ...
    if(isset($data)){
        if($data->message=='Server error' && $data->data=='Unoparty database is behind backend.'){
            $status = 2;
        } else {
            $status = 1;
        }
...
while(1){
    ...
    if($status==1){
        ...
        // If we detected at least 3 errors, fix by restarting unoparty service
        if($errors>=3 && !$restart){
            $errors  = 0;
            $restart = 1;
